        ...
        chatListener = database
            ...
            .addSnapshotListener { [weak self] snapshot, error in
                ...
                let messages: [Message] = objects.compactMap({
                    ...
                    return Message(messageText: $0["text"] as? String ?? "",
                                   messageState: $0["sender"] as? String == self?.currentUsername ? .messageSent : .messageReceived,
                                   messageCreationDate: date
                    )
                }).sorted(by: { first, second in
                    return first.messageCreationDate < second.messageCreationDate
                })
                DispatchQueue.main.async {
